...
const AddSender = () => {
  ...
  const builtUsername = 'Sender-'+random3Digit;
  useEffect(() => {
  ...
  const onCreatePress = async (data) => {
    ...
    try {
      ...
      const NewSenderCredential = await createUserWithEmailAndPassword(auth, email, password);
      const newSender_Uid = NewSenderCredential.user.uid;
      //log back with the current Sender
      await signInWithEmailAndPassword(auth, adminEmail, adminPassword);

      await setDoc(doc(db, "Sender", `${newSender_Uid}`), {
      ...
      navigation.push(`/developerAccess/AddSender/ThankYou?senderName=${data.name}`);
    } catch (error) {
      console.error(error);
